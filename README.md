...
- O `sw.js` usa **network-first para navegação** (HTML da rede quando possível, com fallback em cache offline) e **cache com versão** para ativos estáticos; o nome do cache inclui `v9` — incrementar ao alterar ficheiros críticos para forçar atualização em clientes antigos.
